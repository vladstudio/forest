...
| `tetra`        | no       | —                          | AI integration via Tetra. **Presence opts in** to AI features — even `"tetra": {}` enables it. `port` (default `24100`) is the Tetra HTTP port. `commands.commit` and `commands.pr` map Forest actions to Tetra prompt filenames in `~/.config/tetra/commands/`; both default to `AI Generate commit message` and `AI Generate PR description`. See [AI Integration](#ai-integration) below. |
...
3. Set `"tetra": {}` in your `.forest/config.json` (or `local.json`) to opt in with defaults. To customize, e.g. `{"tetra": {"port": 5000, "commands": {"commit": "My commit prompt", "pr": "My PR prompt"}}}`
