...
export async function createTree(opts: {
  ...
  return await vscode.window.withProgress(
    ...
    async (progress) => {
      ...
      const ports = portManager.resolvePorts(portBase);
      ...
      for (const cmd of cmds) {
        try {
          await exec(cmd, { cwd: treePath, timeout: 120_000 });
        } catch (e: any) {
          vscode.window.showWarningMessage(`Setup command failed: ${e.message}. You can run 'Water' to retry.`);
        }
      }
