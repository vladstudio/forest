...
import { copyConfigFiles } from '../commands/shared';
...
export class ForestWebviewProvider implements vscode.WebviewViewProvider {
  ...
  private async handleMessage(msg: { command: string; key?: string; isDoneOrClosed?: boolean }): Promise<void> {
...
window.addEventListener('message', e => { if (e.data.type === 'update') render(e.data.data); });
