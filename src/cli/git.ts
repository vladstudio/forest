...
export async function createWorktree(
  ...
  // Publish the new branch so it has a correct upstream from the start.
  // worktree add inherits tracking from the start point (e.g. origin/main),
  // so if the publish fails, unset that to prevent bare git-push from
  // accidentally targeting the base branch.
  try {
