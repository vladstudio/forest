...
- **ShortcutManager**: Terminal/browser lifecycle. Tracks terminals in `Map<string, vscode.Terminal[]>`. Emits change events for UI. `openWith()` shows a QuickPick to choose from the configured app list. External terminal support for iTerm, Terminal.app, and Ghostty via AppleScript.
